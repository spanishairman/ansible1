...
NAT-подключения для управления этим хостом. Можно добавить в файл Vagrantfile информацию о дополнительных сетевых
интерфейсах, например так:
```
...
```
##### Решение
Для того, чтобы обойти возникшую проблему, будем использовать сеть управления в качество основной и единственной.
...
резервирования для неё ip-адреса за диапазоном выдаваемых адресов, используем командный [файл](nano vagrant-net-reload.sh) со следующим содержимым:
```
#!/bin/bash
...
Все изменения, вносимые таким образом, применяются "на лету" и не требуют перезапуска виртуельной сети.
...
в нём мы указали имя уже созданной сети "vagrant-libvirt-mgmt", в которой заданы:
  - адресация, 
  - mac-адрес сетевого интерфейса создаваемой виртуальной машины,
